...
class BaseModel():
    ...
    def resume_training(self, resume_state):
        ...
        for i, s in enumerate(resume_schedulers):
         try:
            # 只加载last_epoch等关键字段，避免配置冲突
            current_scheduler = self.schedulers[i]
            resume_scheduler = s
            
            # 只复制last_epoch，保持其他配置不变
            if hasattr(current_scheduler, 'last_epoch') and 'last_epoch' in resume_scheduler:
                current_scheduler.last_epoch = resume_scheduler['last_epoch']
                print(f"Scheduler {i}: restored last_epoch to {resume_scheduler['last_epoch']}")
            
            # 对于CosineAnnealingRestartCyclicLR，还可以选择性恢复其他字段
            if hasattr(current_scheduler, '_step_count') and '_step_count' in resume_scheduler:
                current_scheduler._step_count = resume_scheduler['_step_count']
                
         except Exception as e:
            print(f"Warning: Failed to partially load scheduler {i}: {e}")
            print("Using new scheduler configuration instead")
